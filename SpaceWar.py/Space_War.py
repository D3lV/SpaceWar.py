...
cover_image = pygame.image.load('images/newinicio.png').convert()
...
def game_over():
    global score, lives
    message = game_over_font.render(f"Game Over! Puntos: {score}", True, RED)
...
def show_start_screen():
    ...
    screen.fill(BLACK)
    title = title_font.render("SPACE WAR", True, RED)
    ...
    botton_left_text = font.render("Juego desarrollado para mejorar las habilidades de los programadores", True, WHITE)
    ...
    github_button = font.render("D3LV®-CODE", True, RED)
...
# Mostrar texto en la esquina inferior izquierda
    botton_left_text_rect = botton_left_text.get_rect(topleft=(190, height - 30))
    screen.blit(botton_left_text, botton_left_text_rect)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if github_rect.collidepoint(event.pos):
                    webbrowser.open("https://github.com/D3lV/SpaceWar.py")
